...
- (void)createRepairsStationTableViewCell{
    //图片
    _stationImageView = [[UIImageView alloc]initWithFrame:CGRectMake(30 * screenWidth, 20 * screenHeight, 110 * screenWidth, 110 * screenHeight)];
    _stationImageView.image = [UIImage imageNamed:@""];
    [self.contentView addSubview:_stationImageView];
    //名称
    _stationTitle = [[UILabel alloc]initWithFrame:CGRectMake(_stationImageView.frame.size.width + _stationImageView.frame.origin.x + 20 * screenWidth, _stationImageView.frame.origin.y, [UIScreen mainScreen].bounds.size.width - 176 * screenWidth, 30 * screenHeight)];
    [_stationTitle setFont:[UIFont fontWithName:@"Helvetica-Bold" size:16]];
    _stationTitle.text = @"";
    ...
    _stationStarLabel.font = CFFONT14;
    ...
    _stationRecommend.font = CFFONT13;
...
- (void)setModel:(CFRepairsStationModel *)model{
    ...
    if ([model.county isEqualToString:@"county"]) {
        model.county = @"";
    }
