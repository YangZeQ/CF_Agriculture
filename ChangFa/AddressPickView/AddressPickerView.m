...
- (instancetype)initWithFrame:(CGRect)frame
                PickViewStyle:(pickViewStyle)style{
    self = [super initWithFrame:frame];
    if (self) {
        //加载地址数据源
//        switch (style) {
...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        ...
        NSLog(@"%f", _addressPickerView.frame.size.height);
...
- (NSString *)pickerView:(UIPickerView *)pickerView
    ...
    } else if (3 == _viewStyle) {
        if (0 == component) {
            ProvinceModel * province = _provinceArray[row];
