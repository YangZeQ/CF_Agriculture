//
//  CFAgencyReturnStatusViewController.m
//  ChangFa
//
...
//    self.retreatScrollView.pagingEnabled = YES;
    ...
    titleStastusLabel.textColor = [UIColor redColor];
...
//            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[[UIImage imageNamed:@"diangengduo"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStyleDone target:self action:@selector(rightButtonClick)];
        ...
        case 2:
            ...
            textStastusLabel.text = @"理由：返厂农机不符合审核条件";
            timeStastuslabel.text = [[_model.checkDate substringWithRange:NSMakeRange(0, 19)] stringByReplacingOccurrencesOfString:@"T" withString:@" "];
            break;
        default:
            break;
    ...
    UIView *retreatInfoView = [[UIView alloc]initWithFrame:CGRectMake(30 * screenWidth, statiusView.frame.size.height + 20 * screenHeight, self.view.frame.size.width - 30 * 2 * screenWidth, 970 * screenHeight)];
    ...
    UILabel *retreatReason = [[UILabel alloc]initWithFrame:CGRectMake(machineName.frame.origin.x, machineNumber.frame.size.height + machineNumber.frame.origin.y + 50 * screenHeight, machineName.frame.size.width, machineName.frame.size.height)];
    ...
    UILabel *retreatReasonLabel = [[UILabel alloc]initWithFrame:CGRectMake(machineName.frame.origin.x, reasonDescribe.frame.size.height + reasonDescribe.frame.origin.y + 10 * screenHeight, machineName.frame.size.width + 10 * screenWidth, 50 * screenHeight)];
    ...
    retreatReason.text = @"返厂原因";
    reasonDescribe.text = self.model.reason;
    retreatReasonLabel.text = self.model.note;
    
    CGFloat height = [UILabel getHeightByWidth:retreatReasonLabel.frame.size.width title:retreatReasonLabel.text font:retreatReasonLabel.font];
    retreatReasonLabel.frame = CGRectMake(retreatReasonLabel.frame.origin.x, retreatReasonLabel.frame.origin.y, retreatReasonLabel.frame.size.width, height);
